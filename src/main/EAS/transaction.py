...
class TransactionResult:
    ...
    @classmethod
    def success_from_receipt(cls, tx_hash: str, receipt: Dict[str, Any]) -> 'TransactionResult':
    ...
    def wait_for_confirmation(self, web3: Web3, timeout: int = 120) -> 'TransactionResult':
        """Wait for transaction confirmation and update receipt."""
        if not self.success:
            return self
        
        try:
            receipt = web3.eth.wait_for_transaction_receipt(self.tx_hash, timeout=timeout)
    ...
    def get_events(self, contract_abi: List[Dict], event_name: Optional[str] = None) -> List[Dict[str, Any]]:

...
def get_network_config(network_name: str) -> Dict[str, Any]:
    ...
    if network_name not in SUPPORTED_CHAINS:
        ...
        mainnet_networks = [name for name, config in SUPPORTED_CHAINS.items() if config.get('network_type') == 'mainnet']
        testnet_networks = [name for name, config in SUPPORTED_CHAINS.items() if config.get('network_type') == 'testnet']
    ...
    if not _verify_contract_integrity(config, network_name):
        raise SecurityError(f"Contract address integrity check failed for {network_name}")
...
def get_mainnet_chains() -> List[str]:
    ...
    """
    mainnet_chains = [
        name for name, config in SUPPORTED_CHAINS.items() 
...
def validate_chain_config(config: Dict[str, Any], chain_name: str) -> None:
    ...
    for field, expected_type in required_fields.items():
        if field not in config:
            raise ValueError(f"Missing required field '{field}' in {chain_name} configuration")
        ...
        try:
            if field == 'contract_address':
                ...
                SecureEnvironmentValidator.validate_rpc_url(value, require_https=require_https)
            ...
            raise SecurityError(f"Security validation failed for {field} in {chain_name}: {str(e)}")
...
def _verify_contract_integrity(config: Dict[str, Any], chain_name: str) -> bool:
    ...
    try:
        ...
        return ContractAddressValidator.is_valid_eas_contract(contract_address, chain_id)
...
def create_eas_instance(network_name: str = None, from_account: str = None, private_key: str = None, rpc_url: str = None):
    ...
    if not from_account or not private_key:
        raise ValueError("FROM_ACCOUNT and PRIVATE_KEY must be provided or set as environment variables")
    ...
    try:
        ...
        if rpc_url:
            ...
            rpc_url = SecureEnvironmentValidator.validate_rpc_url(os.getenv("RPC_URL"))
    ...
    if os.getenv("CONTRACT_ADDRESS"):
        try:
            contract_addr = SecureEnvironmentValidator.validate_address(os.getenv("CONTRACT_ADDRESS"))

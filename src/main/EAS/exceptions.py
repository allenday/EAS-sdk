    ...
    def _sanitize_context(context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in context.items():
            if key in sensitive_fields and value:
                sanitized_context[key] = SecureEnvironmentValidator.sanitize_for_logging(
    ...
    def __init__(self, message: str, field_name: Optional[str] = None, field_value: Any = None):
        ...
        if field_value is not None:
            ...
            if isinstance(field_value, str):
                ...
                elif field_name in ['address', 'from_account', 'to_account', 'recipient']:
                    from .security import SecureEnvironmentValidator
                    context['field_value'] = SecureEnvironmentValidator.sanitize_for_logging(field_value, "address")
...
class EASTransactionError(EASError):
    ...
    def __init__(self, message: str, tx_hash: Optional[str] = None, receipt: Optional[dict] = None):
        ...
        if tx_hash:
            ...
            from .security import SecureEnvironmentValidator
            context['tx_hash'] = SecureEnvironmentValidator.sanitize_for_logging(tx_hash, "transaction_hash")
        if receipt:
            ...
            context['transaction_status'] = receipt.get('status')
...
class EASNetworkError(EASError):
    ...
    def __init__(self, message: str, rpc_url: Optional[str] = None, network_name: Optional[str] = None):
        ...
        if rpc_url:
            ...
            from .security import SecureEnvironmentValidator
            context['rpc_url'] = SecureEnvironmentValidator.sanitize_for_logging(rpc_url, "url")
        if network_name:
...
class EASContractError(EASError):
    ...
    def __init__(self, message: str, contract_address: Optional[str] = None, method_name: Optional[str] = None):
        ...
        if contract_address:
            ...
            from .security import SecureEnvironmentValidator
            context['contract_address'] = SecureEnvironmentValidator.sanitize_for_logging(contract_address, "address")
        if method_name:
...
class EASSecurityError(EASError):
    ...
    def __init__(self, message: str, threat_type: Optional[str] = None, severity: str = "high"):
        context = {
            'security_event': True,
            'severity': severity
        }

...
from .type_parser import EASTypeParser, EASField
from .schema_generator import generate_proto_format
...
def resolve_protobuf_type(schema_uid: str, namespace: str = "vendor.v1") -> type:
    ...
    try:
        ...
        raise ImportError(f"Protobuf module not found: main.EAS.generated.{module_path}_pb2")
    ...
    try:
        ...
        raise AttributeError(f"Message class '{message_name}' not found in module '{module_path}'")


def encode_data_protobuf(data: Dict[str, Any], message_class: type, output_format: str = "binary") -> Union[bytes, str]:
    """
...
def encode_schema_data(schema_uid: str, data: Dict[str, Any], 
    ...
    if format_type == "protobuf":
        ...
        if message_type:
            ...
            try:
                ...
                raise AttributeError(f"Message class '{class_name}' not found in module '{module_path}'")
        else:
            # Use namespace and schema_uid to resolve type
            message_class = resolve_protobuf_type(schema_uid, namespace)
...
def decode_schema_data(encoded_data: Union[bytes, str], message_class: type, 
                      input_format: str = "binary") -> Dict[str, Any]:
    """

...
def _parse_field_value(data_bytes: bytes, offset: int, field_type: EASType) -> tuple[Any, int]:
    ...
    if base_type == "address":
        return _parse_address_array(data_bytes, offset, dimensions) if is_array else _parse_address(data_bytes, offset)
    elif base_type == "string":
        return _parse_string_array(data_bytes, offset, dimensions) if is_array else _parse_string(data_bytes, offset)
    elif base_type == "bool":
        return _parse_bool_array(data_bytes, offset, dimensions) if is_array else _parse_bool(data_bytes, offset)
    elif base_type.startswith(("uint", "int")):
        return _parse_integer_array(data_bytes, offset, field_type, dimensions) if is_array else _parse_integer(data_bytes, offset, field_type)
    elif base_type.startswith("bytes"):
        return _parse_bytes_array(data_bytes, offset, field_type, dimensions) if is_array else _parse_bytes(data_bytes, offset, field_type)
    else:
...
def _parse_address_array(data_bytes: bytes, offset: int, dimensions: List[int]) -> tuple[List[str], int]:
...
def _parse_string_array(data_bytes: bytes, offset: int, dimensions: List[int]) -> tuple[List[str], int]:
...
def _parse_bool_array(data_bytes: bytes, offset: int, dimensions: List[int]) -> tuple[List[bool], int]:
...
def _parse_integer(data_bytes: bytes, offset: int, field_type: EASType) -> tuple[int, int]:
...
def _parse_integer_array(data_bytes: bytes, offset: int, field_type: EASType, dimensions: List[int]) -> tuple[List[int], int]:
    ...
    if not dimensions:
        ...
        current_offset = array_offset + 32
        for _ in range(length):
            integer, current_offset = _parse_integer(data_bytes, current_offset, field_type)
...
def _parse_bytes(data_bytes: bytes, offset: int, field_type: EASType) -> tuple[str, int]:
    """Parse a bytes field."""
    base_type = field_type.base_type
...
def _parse_bytes_array(data_bytes: bytes, offset: int, field_type: EASType, dimensions: List[int]) -> tuple[List[str], int]:
    ...
    if not dimensions:
        ...
        current_offset = array_offset + 32
        for _ in range(length):
            bytes_data, current_offset = _parse_bytes(data_bytes, current_offset, field_type)
